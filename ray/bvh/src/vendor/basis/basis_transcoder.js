...
const BASIS = ( function () {
	...
	return (
		function ( BASIS ) {
			...
			const Module = typeof BASIS !== 'undefined' ? BASIS : {}; let readyPromiseResolve, readyPromiseReject; Module[ 'ready' ] = new Promise( function ( resolve, reject ) {
			...
			let ENVIRONMENT_IS_WEB = false; let ENVIRONMENT_IS_WORKER = false; let ENVIRONMENT_IS_NODE = false; let ENVIRONMENT_IS_SHELL = false; ENVIRONMENT_IS_WEB = typeof window === 'object'; ENVIRONMENT_IS_WORKER = typeof importScripts === 'function'; ENVIRONMENT_IS_NODE = typeof process === 'object' && typeof process.versions === 'object' && typeof process.versions.node === 'string'; ENVIRONMENT_IS_SHELL = ! ENVIRONMENT_IS_WEB && ! ENVIRONMENT_IS_NODE && ! ENVIRONMENT_IS_WORKER; let scriptDirectory = ''; function locateFile( path ) {
			...
			let read_, readAsync, readBinary, setWindowTitle; let nodeFS; let nodePath; if ( ENVIRONMENT_IS_NODE ) {
			...
			const out = Module[ 'print' ] || console.log.bind( console ); const err = Module[ 'printErr' ] || console.warn.bind( console ); for ( key in moduleOverrides ) {
			...
			moduleOverrides = null; if ( Module[ 'arguments' ] )arguments_ = Module[ 'arguments' ]; if ( Module[ 'thisProgram' ] )thisProgram = Module[ 'thisProgram' ]; if ( Module[ 'quit' ] )quit_ = Module[ 'quit' ]; let tempRet0 = 0; const setTempRet0 = function ( value ) {

				tempRet0 = value;

			};

			let wasmBinary; if ( Module[ 'wasmBinary' ] )wasmBinary = Module[ 'wasmBinary' ]; let noExitRuntime; if ( Module[ 'noExitRuntime' ] )noExitRuntime = Module[ 'noExitRuntime' ]; if ( typeof WebAssembly !== 'object' ) {
			...
			const UTF8Decoder = typeof TextDecoder !== 'undefined' ? new TextDecoder( 'utf8' ) : undefined; function UTF8ArrayToString( heap, idx, maxBytesToRead ) {
				...
				const endIdx = idx + maxBytesToRead; let endPtr = idx; while ( heap[ endPtr ] && ! ( endPtr >= endIdx ) )++ endPtr; if ( endPtr - idx > 16 && heap.subarray && UTF8Decoder ) {
			...
			function lengthBytesUTF8( str ) {
				...
				let len = 0; for ( let i = 0; i < str.length; ++ i ) {
					...
					let u = str.charCodeAt( i ); if ( u >= 55296 && u <= 57343 )u = 65536 + ( ( u & 1023 ) << 10 ) | str.charCodeAt( ++ i ) & 1023; if ( u <= 127 )++ len; else if ( u <= 2047 )len += 2; else if ( u <= 65535 )len += 3; else len += 4;
			...
			const UTF16Decoder = typeof TextDecoder !== 'undefined' ? new TextDecoder( 'utf-16le' ) : undefined; function UTF16ToString( ptr, maxBytesToRead ) {

				let endPtr = ptr; let idx = endPtr >> 1; const maxIdx = idx + maxBytesToRead / 2; while ( ! ( idx >= maxIdx ) && HEAPU16[ idx ] )++ idx; endPtr = idx << 1; if ( endPtr - ptr > 32 && UTF16Decoder ) {
			...
			function stringToUTF16( str, outPtr, maxBytesToWrite ) {
				...
				if ( maxBytesToWrite < 2 ) return 0; maxBytesToWrite -= 2; const startPtr = outPtr; const numCharsToWrite = maxBytesToWrite < str.length * 2 ? maxBytesToWrite / 2 : str.length; for ( let i = 0; i < numCharsToWrite; ++ i ) {
			...
			function stringToUTF32( str, outPtr, maxBytesToWrite ) {
				...
				if ( maxBytesToWrite < 4 ) return 0; const startPtr = outPtr; const endPtr = startPtr + maxBytesToWrite - 4; for ( let i = 0; i < str.length; ++ i ) {
			...
			const INITIAL_MEMORY = Module[ 'INITIAL_MEMORY' ] || 16777216; let wasmTable; const __ATPRERUN__ = []; const __ATINIT__ = []; const __ATMAIN__ = []; const __ATPOSTRUN__ = []; let runtimeInitialized = false; function preRun() {
			...
			Module[ 'preloadedImages' ] = {}; Module[ 'preloadedAudios' ] = {}; function abort( what ) {
				...
				what += ''; err( what ); ABORT = true; EXITSTATUS = 1; what = 'abort(' + what + '). Build with -s ASSERTIONS=1 for more info.'; const e = new WebAssembly.RuntimeError( what ); readyPromiseReject( e ); throw e;
			...
			const fileURIPrefix = 'file://'; function isFileURI( filename ) {

				return hasPrefix( filename, fileURIPrefix );

			}

			let wasmBinaryFile = 'basis_transcoder.wasm'; if ( ! isDataURI( wasmBinaryFile ) ) {
			...
			function createWasm() {
				...
				const info = { 'a': asmLibraryArg }; function receiveInstance( instance, module ) {
					...
					const exports = instance.exports; Module[ 'asm' ] = exports; wasmMemory = Module[ 'asm' ][ 'K' ]; updateGlobalBufferAndViews( wasmMemory.buffer ); wasmTable = Module[ 'asm' ][ 'L' ]; removeRunDependency( 'wasm-instantiate' );
			...
			const structRegistrations = {}; function runDestructors( destructors ) {
			...
			const awaitingDependencies = {}; const registeredTypes = {}; const typeDependencies = {}; const char_0 = 48; const char_9 = 57; function makeLegalFunctionName( name ) {
			...
			let InternalError = undefined; function throwInternalError( message ) {
			...
			function whenDependentTypesAreResolved( myTypes, dependentTypes, getTypeConverters ) {
				...
				const typeConverters = new Array( dependentTypes.length ); const unregisteredTypes = []; let registered = 0; dependentTypes.forEach( function ( dt, i ) {
			...
			function __embind_finalize_value_object( structType ) {
				...
				const reg = structRegistrations[ structType ]; delete structRegistrations[ structType ]; const rawConstructor = reg.rawConstructor; const rawDestructor = reg.rawDestructor; const fieldRecords = reg.fields; const fieldTypes = fieldRecords.map( function ( field ) {
					...
					const fields = {}; fieldRecords.forEach( function ( field, i ) {

						const fieldName = field.fieldName; const getterReturnType = fieldTypes[ i ]; const getter = field.getter; const getterContext = field.getterContext; const setterArgumentType = fieldTypes[ i + fieldRecords.length ]; const setter = field.setter; const setterContext = field.setterContext; fields[ fieldName ] = { read: function ( ptr ) {
			...
			let BindingError = undefined; function throwBindingError( message ) {
			...
			function registerType( rawType, registeredInstance, options ) {
				...
				registeredTypes[ rawType ] = registeredInstance; delete typeDependencies[ rawType ]; if ( awaitingDependencies.hasOwnProperty( rawType ) ) {
					...
					const callbacks = awaitingDependencies[ rawType ]; delete awaitingDependencies[ rawType ]; callbacks.forEach( function ( cb ) {
			...
			function __embind_register_bool( rawType, name, size, trueValue, falseValue ) {
				...
				const shift = getShiftFromSize( size ); name = readLatin1String( name ); registerType( rawType, { name: name, 'fromWireType': function ( wt ) {
			...
			function ClassHandle_isAliasOf( other ) {
				...
				let leftClass = this.$$.ptrType.registeredClass; let left = this.$$.ptr; let rightClass = other.$$.ptrType.registeredClass; let right = other.$$.ptr; while ( leftClass.baseClass ) {
			...
			let finalizationGroup = false; function detachFinalizer( handle ) {}
			...
			let delayFunction = undefined; const deletionQueue = []; function flushPendingDeletes() {
			...
			const registeredPointers = {}; function ensureOverloadTable( proto, methodName, humanName ) {

				if ( undefined === proto[ methodName ].overloadTable ) {

					const prevFunc = proto[ methodName ]; proto[ methodName ] = function () {
			...
			function genericPointerToWireType( destructors, handle ) {
				...
				const handleClass = handle.$$.ptrType.registeredClass; ptr = upcastPointer( handle.$$.ptr, handleClass, this.registeredClass ); if ( this.isSmartPointer ) {
					...
					case 0:if ( handle.$$.smartPtrType === this ) {
						...
						const clonedHandle = handle[ 'clone' ](); ptr = this.rawShare( ptr, __emval_register( function () {
			...
			function RegisteredPointer_fromWireType( ptr ) {
				...
				const dp = downcastPointer( rawPointer, this.registeredClass, toType.registeredClass ); if ( dp === null ) {
			...
			function getDynCaller( sig, ptr ) {
				...
				assert( sig.indexOf( 'j' ) >= 0, 'getDynCaller should only be called with i64 sigs' ); const argCache = []; return function () {

					argCache.length = arguments.length; for ( let i = 0; i < arguments.length; i ++ ) {
			...
			let UnboundTypeError = undefined; function getTypeName( type ) {
			...
			function __embind_register_class( rawType, rawPointerType, rawConstPointerType, baseClassRawType, getActualTypeSignature, getActualType, upcastSignature, upcast, downcastSignature, downcast, name, destructorSignature, rawDestructor ) {
				...
				rawDestructor = embind__requireFunction( destructorSignature, rawDestructor ); const legalFunctionName = makeLegalFunctionName( name ); exposePublicSymbol( legalFunctionName, function () {
					...
					const constructor = createNamedFunction( legalFunctionName, function () {
						...
						const body = registeredClass.constructor_body[ arguments.length ]; if ( undefined === body ) {
			...
			function __embind_register_class_constructor( rawClassType, argCount, rawArgTypesAddr, invokerSignature, invoker, rawConstructor ) {
				...
				assert( argCount > 0 ); const rawArgTypes = heap32VectorToArray( argCount, rawArgTypesAddr ); invoker = embind__requireFunction( invokerSignature, invoker ); const args = [ rawConstructor ]; const destructors = []; whenDependentTypesAreResolved( [], [ rawClassType ], function ( classType ) {
					...
					whenDependentTypesAreResolved( [], rawArgTypes, function ( argTypes ) {
						...
						classType.registeredClass.constructor_body[ argCount - 1 ] = function constructor_body() {
							...
							destructors.length = 0; args.length = argCount; for ( let i = 1; i < argCount; ++ i ) {

								args[ i ] = argTypes[ i ][ 'toWireType' ]( destructors, arguments[ i - 1 ] );

							}

							const ptr = invoker.apply( null, args ); runDestructors( destructors ); return argTypes[ 0 ][ 'fromWireType' ]( ptr );
			...
			function craftInvokerFunction( humanName, argTypes, classType, cppInvokerFunc, cppTargetFunc ) {
				...
				const isClassMethodFunc = argTypes[ 1 ] !== null && classType !== null; let needsDestructorStack = false; for ( var i = 1; i < argTypes.length; ++ i ) {
				...
				const returns = argTypes[ 0 ].name !== 'void'; let argsList = ''; let argsListWired = ''; for ( var i = 0; i < argCount - 2; ++ i ) {
				...
				const dtorStack = needsDestructorStack ? 'destructors' : 'null'; const args1 = [ 'throwBindingError', 'invoker', 'fn', 'runDestructors', 'retType', 'classParam' ]; const args2 = [ throwBindingError, cppInvokerFunc, cppTargetFunc, runDestructors, argTypes[ 0 ], argTypes[ 1 ] ]; if ( isClassMethodFunc ) {
				...
				invokerFnBody += '}\n'; args1.push( invokerFnBody ); const invokerFunction = new_( Function, args1 ).apply( null, args2 ); return invokerFunction;

			}

			function __embind_register_class_function( rawClassType, methodName, argCount, rawArgTypesAddr, invokerSignature, rawInvoker, context, isPureVirtual ) {

				const rawArgTypes = heap32VectorToArray( argCount, rawArgTypesAddr ); methodName = readLatin1String( methodName ); rawInvoker = embind__requireFunction( invokerSignature, rawInvoker ); whenDependentTypesAreResolved( [], [ rawClassType ], function ( classType ) {

					classType = classType[ 0 ]; const humanName = classType.name + '.' + methodName; if ( isPureVirtual ) {
					...
					whenDependentTypesAreResolved( [], rawArgTypes, function ( argTypes ) {
						...
						const memberFunction = craftInvokerFunction( humanName, argTypes, classType, rawInvoker, context ); if ( undefined === proto[ methodName ].overloadTable ) {
			...
			const emval_free_list = []; const emval_handle_array = [ {}, { value: undefined }, { value: null }, { value: true }, { value: false } ]; function __emval_decref( handle ) {
			...
			function count_emval_handles() {
				...
				let count = 0; for ( let i = 5; i < emval_handle_array.length; ++ i ) {
			...
			function __emval_register( value ) {
				...
				default:{
					...
					const handle = emval_free_list.length ? emval_free_list.pop() : emval_handle_array.length; emval_handle_array[ handle ] = { refcount: 1, value: value }; return handle;
			...
			function __embind_register_emval( rawType, name ) {
				...
				name = readLatin1String( name ); registerType( rawType, { name: name, 'fromWireType': function ( handle ) {
					...
					const rv = emval_handle_array[ handle ].value; __emval_decref( handle ); return rv;
			...
			function enumReadValueFromPointer( name, shift, signed ) {
				...
				case 0:return function ( pointer ) {
					...
					const heap = signed ? HEAP8 : HEAPU8; return this[ 'fromWireType' ]( heap[ pointer ] );

				};

				case 1:return function ( pointer ) {

					const heap = signed ? HEAP16 : HEAPU16; return this[ 'fromWireType' ]( heap[ pointer >> 1 ] );

				};

				case 2:return function ( pointer ) {

					const heap = signed ? HEAP32 : HEAPU32; return this[ 'fromWireType' ]( heap[ pointer >> 2 ] );
			...
			function __embind_register_enum( rawType, name, size, isSigned ) {
				...
				const shift = getShiftFromSize( size ); name = readLatin1String( name ); function ctor() {}
			...
			function _embind_repr( v ) {
				...
				const t = typeof v; if ( t === 'object' || t === 'array' || t === 'function' ) {
			...
			function __embind_register_float( rawType, name, size ) {
				...
				const shift = getShiftFromSize( size ); name = readLatin1String( name ); registerType( rawType, { name: name, 'fromWireType': function ( value ) {
			...
			function __embind_register_function( name, argCount, rawArgTypesAddr, signature, rawInvoker, fn ) {
				...
				const argTypes = heap32VectorToArray( argCount, rawArgTypesAddr ); name = readLatin1String( name ); rawInvoker = embind__requireFunction( signature, rawInvoker ); exposePublicSymbol( name, function () {

					throwUnboundTypeError( 'Cannot call ' + name + ' due to unbound types', argTypes );

				}, argCount - 1 ); whenDependentTypesAreResolved( [], argTypes, function ( argTypes ) {

					const invokerArgsArray = [ argTypes[ 0 ], null ].concat( argTypes.slice( 1 ) ); replacePublicSymbol( name, craftInvokerFunction( name, invokerArgsArray, null, rawInvoker, fn ), argCount - 1 ); return [];
			...
			function __embind_register_integer( primitiveType, name, size, minRange, maxRange ) {
				...
				const shift = getShiftFromSize( size ); let fromWireType = function ( value ) {
				...
				if ( minRange === 0 ) {
					...
					const bitshift = 32 - 8 * size; fromWireType = function ( value ) {
				...
				const isUnsignedType = name.indexOf( 'unsigned' ) != - 1; registerType( primitiveType, { name: name, 'fromWireType': fromWireType, 'toWireType': function ( destructors, value ) {
			...
			function __embind_register_memory_view( rawType, dataTypeIndex, name ) {
				...
				const typeMapping = [ Int8Array, Uint8Array, Int16Array, Uint16Array, Int32Array, Uint32Array, Float32Array, Float64Array ]; const TA = typeMapping[ dataTypeIndex ]; function decodeMemoryView( handle ) {
					...
					handle = handle >> 2; const heap = HEAPU32; const size = heap[ handle ]; const data = heap[ handle + 1 ]; return new TA( buffer, data, size );
			...
			function __embind_register_std_string( rawType, name ) {
				...
				name = readLatin1String( name ); const stdStringIsUTF8 = name === 'std::string'; registerType( rawType, { name: name, 'fromWireType': function ( value ) {

					const length = HEAPU32[ value >> 2 ]; let str; if ( stdStringIsUTF8 ) {

						let decodeStartPtr = value + 4; for ( var i = 0; i <= length; ++ i ) {

							const currentBytePtr = value + 4 + i; if ( i == length || HEAPU8[ currentBytePtr ] == 0 ) {
					...
					let getLength; const valueIsOfTypeString = typeof value === 'string'; if ( ! ( valueIsOfTypeString || value instanceof Uint8Array || value instanceof Uint8ClampedArray || value instanceof Int8Array ) ) {
					...
					const length = getLength(); const ptr = _malloc( 4 + length + 1 ); HEAPU32[ ptr >> 2 ] = length; if ( stdStringIsUTF8 && valueIsOfTypeString ) {
			...
			function __embind_register_std_wstring( rawType, charSize, name ) {
				...
				name = readLatin1String( name ); let decodeString, encodeString, getHeap, lengthBytesUTF, shift; if ( charSize === 2 ) {
				...
				registerType( rawType, { name: name, 'fromWireType': function ( value ) {
					...
					const length = HEAPU32[ value >> 2 ]; const HEAP = getHeap(); let str; let decodeStartPtr = value + 4; for ( let i = 0; i <= length; ++ i ) {

						const currentBytePtr = value + 4 + i * charSize; if ( i == length || HEAP[ currentBytePtr >> shift ] == 0 ) {
					...
					const length = lengthBytesUTF( value ); const ptr = _malloc( 4 + length + charSize ); HEAPU32[ ptr >> 2 ] = length >> shift; encodeString( value, ptr + 4, length + charSize ); if ( destructors !== null ) {
			...
			function __emval_as( handle, returnType, destructorsRef ) {
				...
				handle = requireHandle( handle ); returnType = requireRegisteredType( returnType, 'emval::as' ); const destructors = []; const rd = __emval_register( destructors ); HEAP32[ destructorsRef >> 2 ] = rd; return returnType[ 'toWireType' ]( destructors, handle );

			}

			const emval_symbols = {}; function getStringOrSymbol( address ) {
			...
			const emval_methodCallers = []; function __emval_call_void_method( caller, handle, methodName, args ) {
			...
			function __emval_get_method_caller( argCount, argTypes ) {
				...
				const types = __emval_lookupTypes( argCount, argTypes ); const retType = types[ 0 ]; const signatureName = retType.name + '_$' + types.slice( 1 ).map( function ( t ) {

					return t.name;

				} ).join( '_' ) + '$'; const params = [ 'retType' ]; const args = [ retType ]; let argsList = ''; for ( var i = 0; i < argCount - 1; ++ i ) {

					argsList += ( i !== 0 ? ', ' : '' ) + 'arg' + i; params.push( 'argType' + i ); args.push( types[ 1 + i ] );

				}

				const functionName = makeLegalFunctionName( 'methodCaller_' + signatureName ); let functionBody = 'return function ' + functionName + '(handle, name, destructors, args) {\n'; let offset = 0; for ( var i = 0; i < argCount - 1; ++ i ) {
				...
				functionBody += '};\n'; params.push( functionBody ); const invokerFunction = new_( Function, params ).apply( null, args ); return __emval_addMethodCaller( invokerFunction );
			...
			function craftEmvalAllocator( argCount ) {
				...
				let argsList = ''; for ( var i = 0; i < argCount; ++ i ) {
			...
			const emval_newers = {}; function __emval_new( handle, argCount, argTypes, args ) {

				handle = requireHandle( handle ); let newer = emval_newers[ argCount ]; if ( ! newer ) {
			...
			function __emval_run_destructors( handle ) {
				...
				const destructors = emval_handle_array[ handle ].value; runDestructors( destructors ); __emval_decref( handle );
			...
			function _emscripten_resize_heap( requestedSize ) {
				...
				requestedSize = requestedSize >>> 0; const oldSize = _emscripten_get_heap_size(); const maxHeapSize = 2147483648; if ( requestedSize > maxHeapSize ) {
				...
				const minHeapSize = 16777216; for ( let cutDown = 1; cutDown <= 4; cutDown *= 2 ) {
					...
					let overGrownHeapSize = oldSize * ( 1 + .2 / cutDown ); overGrownHeapSize = Math.min( overGrownHeapSize, requestedSize + 100663296 ); const newSize = Math.min( maxHeapSize, alignUp( Math.max( minHeapSize, requestedSize, overGrownHeapSize ), 65536 ) ); const replacement = emscripten_realloc_buffer( newSize ); if ( replacement ) {
			...
			var SYSCALLS = { mappings: {}, buffers: [ null, [], []], printChar: function ( stream, curr ) {
				...
				const buffer = SYSCALLS.buffers[ stream ]; if ( curr === 0 || curr === 10 ) {
			...
			function _fd_write( fd, iov, iovcnt, pnum ) {
				...
				let num = 0; for ( let i = 0; i < iovcnt; i ++ ) {
					...
					const ptr = HEAP32[ iov + i * 8 >> 2 ]; const len = HEAP32[ iov + ( i * 8 + 4 ) >> 2 ]; for ( let j = 0; j < len; j ++ ) {
			...
			let calledRun; function ExitStatus( status ) {
